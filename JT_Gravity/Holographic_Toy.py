...
# Bulk Spacetime: Define the range for F and Z
F_min, F_max = -10, 1000
Z_min, Z_max = 1, 60

# Create an array of values for F and Z
F_bulk = np.linspace(F_min, F_max, 200)
Z_bulk = np.linspace(Z_min, Z_max, 100)
...
# Add labels and title for the bulk
ax.set_xlabel('F')
ax.set_ylabel('Z')
ax.set_zlabel('ds^2')
ax.set_title('JT Gravity: Bulk AdS_2 Metric and Boundary Curve')

# Boundary: Define constants for Schwarzian and boundary behavior
...
t_min, t_max = 0, 10000
t_boundary = np.linspace(t_min, t_max, 1000)
...
ax.plot(F_boundary, Z_boundary, color='r', label='Boundary Curve (F(t), Z(t))', linewidth=3)
